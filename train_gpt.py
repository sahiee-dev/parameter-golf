...
class Block(nn.Module):
    ...
    def forward(self, x, x0, q_w, k_w, v_w, out_w, up_w, down_w, cu_seqlens=None, max_seqlen=0):
        ...
        if 7 <= getattr(self, "layer_idx", -1) <= 10:
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids, target_ids, cu_seqlens=None, max_seqlen=0):
        ...
        ngram_tilt_beta = float(os.environ.get("NGRAM_TILT_BETA", 0.0))
        if ngram_tilt_beta > 0.0:
            B, T = input_ids.shape
            V = logits.shape[-1]
            logits_3d = logits.view(B, T, V).float()
            beta_tensor = torch.full((1,), ngram_tilt_beta, device=logits.device, dtype=logits_3d.dtype)
            prefix = input_ids[:, :-1]
            tilt = torch.zeros_like(logits_3d)
            tilt[:, 1:].scatter_add_(2, prefix.unsqueeze(2), beta_tensor.expand(B, T-1, 1))
            logits_3d = logits_3d + tilt
            logits = logits_3d.view(B * T, V)
