...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if Hyperparameters.use_p2_loss:
            ...
            main_loss = -(weights * target_logp).mean()
        else:
...
def eval_val_sliding_ttt(
    ...
    """Legal score-first TTT: score each chunk's windows under inference_mode,
    then train on that chunk. Every token scored BEFORE any weight update."""
    ...
    for ci in range(num_chunks):
        ...
        # --- Phase 1: SCORE this chunk's windows under inference_mode (legal) ---
...
def main() -> None:
    ...
    # Legal score-first TTT (PR #461 recipe) — primary eval path when enabled
    if args.ttt_enabled:
        torch.cuda.synchronize()
