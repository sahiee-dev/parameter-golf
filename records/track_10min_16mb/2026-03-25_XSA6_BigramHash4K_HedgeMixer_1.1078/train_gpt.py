...
def eval_val_sliding_ttt(
    ...
    for ci in range(num_chunks):
        ...
        # Strictly cap TTT eval time to 585s. If exceeded, skip training to finish scoring in time.
        elapsed_eval_time = time.perf_counter() - t0
        time_limit = float(os.environ.get("TTT_TIME_LIMIT", "585.0"))
        
        if not is_last_chunk and ttt_epochs > 0 and elapsed_eval_time < time_limit:
