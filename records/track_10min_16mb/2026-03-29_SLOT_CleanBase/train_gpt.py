...
class Hyperparameters:
    ...
    slot_lr = float(os.environ.get("SLOT_LR", 1e-3))
...
class GPT(nn.Module):
    ...
    def forward_logits_with_slot(self, input_ids: Tensor, slot_vec: Tensor | None = None) -> Tensor:
        """Return logits with optional SLOT vector added to last-layer hidden states."""
        n = self.num_layers
        x = self.tok_emb(input_ids)
        if self.bigram is not None:
            x = x + self.bigram(input_ids)
        ...
        for i in range(self.num_decoder_layers):
            ...
            x, _ = self.blocks[bi](x, x0,
                ...
                v_embed=ve, v0=v0)
        x = self.final_norm(x)
        # SLOT: add learnable output vector to all positions
        if slot_vec is not None:
            x = x + slot_vec.to(dtype=x.dtype)[None, None, :]
        if self.tie_embeddings:
            logits_proj = F.linear(x, self.tok_emb.weight)
        else:
            logits_proj = self.lm_head(x)
...
def eval_val_sliding_slot(
    ...
    # Freeze all model params — SLOT never touches quantized weights
    ...
    # Restore requires_grad for any subsequent use
    for p in base_model.parameters():
