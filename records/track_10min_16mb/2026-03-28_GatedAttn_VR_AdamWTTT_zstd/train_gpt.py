...
def eval_val_sliding_ttt(
    ...
    optimizer = torch.optim.SGD(
        ttt_params, lr=args.ttt_lr, momentum=args.ttt_momentum, nesterov=True
    )
...
def main() -> None:
    ...
    while True:
        ...
        if args.qat_enabled and args.late_qat_threshold > 0 and scale < args.late_qat_threshold and not CastedLinear._qat_enabled:
    ...
    if _USE_ZSTD and quant_blob_disk[:4] == b'\x28\xb5\x2f\xfd':
        dctx = zstd.ZstdDecompressor()
        _buf = io.BytesIO(dctx.decompress(quant_blob_disk))
    else:
        _buf = io.BytesIO(lzma.decompress(quant_blob_disk))
